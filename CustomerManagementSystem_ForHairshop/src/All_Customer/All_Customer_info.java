...
public class All_Customer_info {
	...
	private class MyactionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if(pressed.getText().equals(BtnStr[4])) { // "삭제" 클릭
				new DeleteCustomer(All_Customer_page, O, C_num);
			}
	...
	private class MyMouseAdapter extends MouseAdapter {
		public void mouseClicked(MouseEvent e) {
			if (e.getClickCount() == 2) { //  고객 정보 더블클릭
				...
				new DetailsOfCustomer(All_Customer_page, O[SelectedRow]);
	...
	public void Search(int st, String sw) {
		...
		C_num = C.size();
		Object [][]All_C_info = new Object[C_num][];
	...
	public void ViewDesign() {
		...
		jTable = new JTable(defaultTabM);
		JScrollPane jScrollPane = new JScrollPane(jTable);
		jTable.setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // 단일 선택
